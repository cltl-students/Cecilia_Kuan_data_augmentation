...
def data_split_groups(
    ...
    'NotitieID',
     0.8,
    Split data to train / dev / test using "RANDOM" method, while taking into account groups that should stay together.
...
def data_stratify(
    ...
    'NotitieID',
     0.8,
    Using "STRATIFIED SAMPLING METHOD" to split data to train / dev / test, while taking into account groups that should stay together.
    ...
    """
    # create training set of `train_size`
    skf = StratifiedKFold(n_splits=1, test_size=1-train_size, random_state=19)
    for train_idx, other_idx in skf.split(df[X_col], df[y_col], groups=df[group_col]):
        train = df.iloc[train_idx]
        other = df.iloc[other_idx]

    # the non-train data is split 50/50 into development and test
    # skf = StratifiedKFold(n_splits=1, test_size=0.5, random_state=19)
    # for dev_idx, test_idx in skf.split(other[X_col], other[y_col], groups=other[group_col]):
    #     dev = other.iloc[dev_idx]
    #     test = other.iloc[test_idx]
    
    return train, other #dev, test
